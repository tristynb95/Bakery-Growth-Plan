...
async function executePlanGeneration() {
    const planSummary = DOMElements.modalBox.dataset.planSummary;
    if (!planSummary) {
        console.error("Cannot generate plan, summary is missing.");
        openModal('warning', { title: 'Error', message: 'Could not find the plan summary to generate from.' });
        return;
    }
    ...
    try {
        ...
        handleAIActionPlan(appState, activeSaveDataFunction, planSummary);
    } catch (error) {
...
export async function handleAIActionPlan(appState, saveDataFn, planSummary) {
    ...
    if (savedPlan) {
        ...
        openModal('aiActionPlan_initial');
    }
}
...
export function openModal(type, context = {}) {
    ...
    switch (type) {
        ...
        case 'sharing':
            ...
            DOMElements.modalCancelBtn.textContent = 'Cancel';
            break;
        case 'aiActionPlan_initial':
            DOMElements.modalTitle.textContent = "AI Action Plan";
            DOMElements.modalContent.innerHTML = `<div class="flex flex-col items-center justify-center p-8 text-center">
                                                      <i class="bi bi-robot text-5xl text-gray-400 mb-4"></i>
                                                      <h3 class="text-xl font-bold font-poppins">Generate Your Action Plan</h3>
                                                      <p class="text-gray-600 mt-2 max-w-md">Let our AI partner analyse your goals to create a tactical, month-by-month action plan. You can edit the plan after it has been generated.</p>
                                                  </div>`;
            DOMElements.modalActionBtn.innerHTML = `<i class="bi bi-stars"></i> Generate Plan`;
            DOMElements.modalActionBtn.onclick = executePlanGeneration;
            DOMElements.modalCancelBtn.textContent = 'Not Now';
            break;
