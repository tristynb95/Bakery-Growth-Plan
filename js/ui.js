...
export async function handleAIActionPlan(appState, saveDataFn, planSummary) {
    ...
    if (savedPlan) {
        ...
        try {
            ...
            // Directly transition to the view state without recursion.
            openModal('aiActionPlan_view');
            const modalContent = document.getElementById('modal-content');
            modalContent.innerHTML = `<div id="ai-printable-area" class="editable-action-plan">${cleanedHTML}</div>`;

            // Initialize the undo/redo stack correctly for the new content.
            undoStack = [];
            redoStack = [];
            saveState(); // This will push the 'cleanedHTML' onto the undo stack.
            ...
            if (error.name === 'AbortError') {
                ...
                // Instead of just returning, we should close the modal
