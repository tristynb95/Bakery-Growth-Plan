...
/**
 * Renders the main calendar grid for the currently selected month.
 */
...
/**
 * Shows the event form pre-filled with data for editing.
 * @param {number} index - The index of the event to edit in the day's event array.
 */
...
/**
 * Sets up all event listeners for the calendar modal and its components.
 */
export function setupCalendarEventListeners() {
    ...
    if (saveEventBtn) saveEventBtn.addEventListener('click', async () => {
        ...
        const isAllDay = document.getElementById('event-all-day-toggle').checked;
        const eventData = {
            title: title,
            allDay: isAllDay,
