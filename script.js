...
const firebaseConfig = {
      apiKey: "AIzaSyD6BprZOdxcRjxENXZcwg6HzUQcvEhKHUU",
  authDomain: "gails-bakery-growth-plan.firebaseapp.com",
  projectId: "gails-bakery-growth-plan",
  storageBucket: "gails-bakery-growth-plan.firebasestorage.app",
  messagingSenderId: "162638540693",
  appId: "1:162638540693:web:084c5248f090379e8f909c",
  measurementId: "G-N7ZG0KVMP2"
};
// ====================================================================
    ...
    step: {
        ...
        'm1s4':{
            ...
            html:`<div class="content-card p-8"><h3 class="text-xl font-bold mb-1 gails-red-text">Step 4: Protect the Core</h3><p class="text-gray-600 mb-6">One key behaviour you will protect for each pillar to ensure standards don't slip.</p><div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                ...
                <div><label for="m1s4_place" class="font-semibold block mb-2">PLACE 🏡</label><textarea id="m1s4_place" class="form-input" rows="2" placeholder="e.g., Complete a full bakery walk with the team every morning."></textarea></div>
            </div></div>`
        },
        'm1s5':{
...
function renderStep(stepNum) {
    ...
    document.getElementById('prev-step-btn').onclick = () => changeStep(-1);
    document.getElementById('next-step-btn').onclick = () => changeStep(1);
}
...
async function handleModalAction() {
    ...
    switch(type) {
        case 'create':
            ...
            if (!newPlanName || !appState.currentUser) return;
            try {
                const plansRef = db.collection('users').doc(appState.currentUser.uid).collection('plans');
...
// ENHANCEMENT: Feature 1 - Login on Enter Key
// Added listeners to both email and password fields for the 'keyup' event.
...
DOMElements.sendResetBtn.addEventListener('click', () => {
    ...
    auth.sendPasswordResetEmail(email)
        ...
        .catch((error) => {
            // Still show the generic success message on the UI
            messageContainer.innerHTML = `<p class="auth-success">If an account exists for this email, a password reset link has been sent. Please check your inbox.</p>`;
...
// ENHANCEMENT: Bug Fix 2 - Prevent modal closing on text highlight drag
// Changed the event from 'click' to 'mousedown'.
