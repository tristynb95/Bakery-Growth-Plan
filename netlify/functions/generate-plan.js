...
exports.handler = async function(event, context) {
  ...
  try {
    ...
    const prompt = `
      You are an expert bakery operations manager. Your task is to create a best-practice, tactical action plan for Month ${month} from a manager's 90-day growth plan summary.

      **Primary Goal:** Generate a clean, self-contained HTML structure with a single table for the specified month.

      **Core Instructions:**
      1.  **Language**: You MUST use British English spelling and grammar (e.g., 'organise', 'centre').
      2.  **Analyse & Group**: Analyse the provided plan for Month ${month}. Group similar or related tasks into single, actionable steps. For instance, 'Izzy to complete kitchen training' and 'Izzy to go into kitchen' should become one action like 'Organise kitchen training for Izzy'.
      ...
      **Step-by-Step Thought Process (Chain-of-Thought):**
      1.  First, I will read the entire plan summary to understand the key objectives for Month ${month}.
      2.  Next, for Month ${month}, I will extract the specific tasks and goals.
